...
package body Root_Event.Engine is
  ...
  function Create return Engine_Event_Ptr is
    E: Engine_Event_Ptr := new Engine_Event;
  begin
    E.Time := Random_Time.Random(G);
    E.Fuel := Random_Time.Random(G) mod 100;
    E.Oxygen := Random_Time.Random(G) mod 500;
    return E;
  end Create;

  function Create return Main_Engine_Event_Ptr is
    E: Main_Engine_Event_Ptr := new Main_Engine_Event;
  begin
    E.Time := Random_Time.Random(G);
    E.Fuel := Random_Time.Random(G) mod 100;
    E.Oxygen := Random_Time.Random(G) mod 500;
  ...
  function Create return Aux_Engine_Event_Ptr is
    E: Aux_Engine_Event_Ptr := new Aux_Engine_Event;
  begin
    E.Time := Random_Time.Random(G);
    E.Fuel := Random_Time.Random(G) mod 100;
    E.Oxygen := Random_Time.Random(G) mod 500;
    E.Side := Aux_Engine_ID'Val(Random_Time.Random(G) mod 2);
    return E;
  end Create;
