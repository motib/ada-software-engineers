...
package body Root_Event.Engine is
  ...
  function Create(Params: not null access Parameters) return Engine_Event is
  begin
    return (Time =>   Random_Time.Random(G), 
            Fuel =>   Integer(Random_Time.Random(G) mod 100), 
            Oxygen => Integer(Random_Time.Random(G) mod 500));
  end Create;

  function Create(Params: not null access Parameters) return Main_Engine_Event is
  begin
    return Main_Engine_Event'(Engine_Event'(Create(Params)) with null record);
  end Create;
  
  function Create(Params: not null access Parameters) return Aux_Engine_Event is
  begin
    return (Engine_Event'(Create(Params)) with 
